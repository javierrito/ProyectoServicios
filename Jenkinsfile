pipeline {
    ...
    stages {
        ...
        stage('Testing') {
            steps {
                dir('cypress/') {
                    sh 'docker run --rm --name Cypress -v "D:/cursoMicroservicios/jenkins_home/jenkins_home/workspace/Pruebas/cypress:/e2e" -w /e2e -e Cypress cypress/included:3.4.0'
                }
            }
        }
        stage('tar videos') 
